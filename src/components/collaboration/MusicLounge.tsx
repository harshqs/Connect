  ...
  return (
    <div className="relative z-40 font-sans">
      ...
      <button
        ...
        className={`flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold transition shadow-sm ${
          state.isPlaying
            ? "bg-gradient-to-r from-[#287d67] to-[#4db59d] text-white shadow-emerald-500/20"
            : "bg-slate-800/80 text-slate-200 hover:bg-slate-700 border border-slate-700"
        }`}
      >
        <Music className={`size-3.5 ${state.isPlaying ? "animate-bounce" : ""}`} />
        <span className="max-w-[140px] truncate">{state.title}</span>
        {state.isPlaying && (
      ...
      {isOpen && (
        <div className="absolute right-0 top-11 w-96 rounded-2xl border border-white/10 bg-[#091310]/95 p-4 text-white shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-top-3">
          ...
          <div className="flex items-center justify-between border-b border-white/10 pb-3 mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#8fa79b] mt-0.5">DJ: <span className="font-semibold text-[#7be3c4]">{state.djName}</span></p>
              </div>
            </div>
            <button
          ...
          {/* Player Media View (YouTube or Spotify Embed) */}
          ...
          {/* Currently Playing Track Info & Equalizer */}
